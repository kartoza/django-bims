...
class TestSourceReferenceFilters(TestCase):
    ...
    def test_get_source_reference_filter_empty_cache(self, mock_set, mock_get):
        ...
        with patch('django.contrib.sites.models.Site.objects.get_current', return_value=self.site):
            result = get_source_reference_filter()
    ...
    def test_generate_source_reference_filter(self, mock_get_current, mock_set, mock_filter):
        ...
        generate_source_reference_filter(current_site_id=self.site.id)

        mock_set.assert_called_once_with(
            f'source_reference_filter_{self.site.id}',
            [
                {'id': self.references[0].id, 'reference': f'- | {self.entry.publication_date.year} | Test', 'type': 'Peer-reviewed scientific article'},
                {'id': self.references[1].id, 'reference': ' | 2024 | Document title 0', 'type': 'Published report or thesis'}]
            , timeout=None)
