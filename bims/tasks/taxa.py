...
@shared_task(name="bims.tasks.fetch_iucn_assessments", queue="update")
def fetch_iucn_assessments(taxa_ids: list[int] | None = None, *, batch_size: int = 1000) -> str:
    """
    Harvest (or update) IUCN assessment history for the requested taxa.
    """
    from bims.models import Taxonomy, IUCNAssessment, IUCNStatus
    from bims.utils.iucn import get_iucn_assessments, normalize_iucn_category_code
    ...
    total = qs.count()
    logger.info("Starting IUCN assessment sync for %s taxon record(s).", total)
    ...
    for taxon in qs.iterator(chunk_size=batch_size):
        assessments, sis_id = get_iucn_assessments(taxon=taxon)
        ...
        for assessment in assessments:
            ...
            data = {
                ...
                "sis_taxon_id": assessment.get("sis_taxon_id") or sis_id,
                ...
                "normalized_status": status_obj,
                "url": assessment.get("url") or "",
                "scope_code": scope.get("code") or "",
                "scope_label": scope_label,
                "raw_data": assessment,
            }
            data_by_id[assessment_id] = data
        ...
        created += max(0, all_objects.count() - len(existing_by_id))

        to_update: list[IUCNAssessment] = []
    ...
    dt = time.perf_counter() - t0
    logger.info(
        "IUCN assessment sync finished: %s created, %s updated in %.2f s.",
        created,
        updated,
        dt,
    )
    return f"Created {created}, updated {updated} IUCN assessment record(s)."
